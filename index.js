...
//this is an example of a closure and it is how inheritance works functions can reach out to other functions that they are contained within but the parent function can not go the other way.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const x = arr.map(item => `name: ${item.animal_name}, scientific: ${item.scientific_name}`)
    // console.log(x)
    return x;
  }
  ...
  */
  ...
  function lowerCaseNames(arr){
    const x = arr.map(item => item.animal_name.toLowerCase());
  ...
  */
  ...
  function lowPopulationAnimals(attrs){
    return attrs.filter(item => item.population < 5)
  }
  ...
  */
  ...
  function USApop(array){
    ...
    const x = array.map(item => item.population)
    console.log(x)
...
function greeting(firstName, lastName ){
  ...
  // console.log(greeting('Robert', 'Von Der Becke'))
